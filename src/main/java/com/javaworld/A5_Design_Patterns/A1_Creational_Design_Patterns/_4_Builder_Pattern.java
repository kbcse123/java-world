 ...
 * would have more than a handful of parameters. Builder pattern was introduced to solve some of the
 * problems with Factory and Abstract Factory design patterns.
 * 
 * 	1) Too Many arguments to pass from client program to the Factory class that can be error-prone and
 ...
 * providing a way to build the object step-by-step and provide a method (build) that will actually return
 * the final Object.
 * 
 * Implementation:
 * 	.We should follow the naming convention and if the class name is Computer then builder class should
 * 	be named as ComputerBuilder.
 * 
 * 	.Builder class should have a public constructor with all the required attributes as parameters.
 *
 * 	.Builder class should have methods to set the optional parameters, and it should return the same
 ...
 * 	needed by client program.For this we need to have a public constructor in the class with Builder
 * 	class as an argument. Look the example below.
 * 
 * Notice that Computer class has only getter methods and no default constructor. So the only way to get
 * a Computer object is through the ComputerBuilder class.
 * 
 * Examples in JDK:
...
class ComputerClass {
    ...
    public String toString() {
        ...
        sb.append("HDD = ").append(HDD).append(", RAM = ").append(RAM).append(", CPU = ").append(CPU)
				.append(", GraphicsCardOn = ").append(graphicsCardOn).append(", bluetoothOn = ")
				.append(bluetoothOn);
...
public class _4_Builder_Pattern {
    public static void main(String... args) {
        ...
        ComputerClass comp = new ComputerClass.ComputerBuilder("500 GB", "2 GB", "2.4 GHz")
				.bluetoothOn(true)
