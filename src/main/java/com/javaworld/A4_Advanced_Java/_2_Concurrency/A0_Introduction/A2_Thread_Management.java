 ...
 *         time to the
 * 
 *         nanosecond. However, these sleep times are not guaranteed to be
 *         precise, because they are limited by the facilities provided by the
 *         underlying OS. Also,
 * 
 *         the sleep period can be terminated by interrupts, as we'll see in a
 *         later section. In any case, you cannot assume that invoking sleep
 *         will suspend the
 * 
 *         thread for precisely the time period specified. The below
 ...
 * 
 *         thread responds to an interrupt, but it is very common for the thread
 *         to terminate. A thread sends an interrupt by invoking interrupt on
 *         the Thread object for the thread
 * 
 *         to be interrupted. For the interrupt mechanism to work correctly, the
 ...
 *         that throw
 * 
 *         InterruptedException, it simply returns from the run method after it
 *         catches that exception. For example, suppose the central message loop
 *         in the
 * 
 *         SleepMessages example were in the run method of a thread's Runnable
 ...
 *         When a thread checks for
 * 
 *         an interrupt by invoking the static method Thread.interrupted,
 *         interrupt status is cleared. The non-static isInterrupted method,
 *         which is used by one thread to
 * 
 ...
 *         However, it's always possible that interrupt
 * 
 *         status will immediately be set again, by another thread invoking
 *         interrupt.
 * 
 *         Joins:
 * 
 ...
 *         then t.join(); causes the current
 * 
 *         thread to pause execution until t's thread terminates. Overloads of
 *         join allow the programmer to specify a waiting period. However, as
 *         with sleep, join is dependent
 * 
 *         on the OS for timing, so you should not assume that join will wait
 ...
 * 
 * 
 *         Note: It is recommended that applications not use wait, notify, or
 *         notifyAll on Thread instances. On a Thread instance, the methods
 *         wait(...), notify() and notifyAll()
 * 
 *         are available only because all classes in Java extend Object and
 *         therefore automatically inherit the methods. But there are two very
 *         good reasons to not call
 * 
 *         these methods on a Thread instance:
 * 
 * 
 * 
 *         1)Doing so is really confusing. What is really expected when calling,
 *         for instance, the wait(...) method on a Thread? That the execution of
 *         the Thread is
 * 
 *         suspended, or that acquisition of the object monitor is waited for?
 * 
 * 
 * 
 *         2)Internally, the JVM relies on these methods to change the state of
 *         the Thread (BLOCKED, WAITING, ...), so calling them will corrupt the
 *         behavior of the JVM.
 * 
 *         Must read:
 *         https://stackoverflow.com/questions/35631955/why-cant-notifyall-be-
 *         used-in-a-thread-instance
 * 
 *         https://stackoverflow.com/questions/4912165/whats-the-best-
 *         alternative-to-wait-notify-for-low-level-synchronization
 * 
 * 
 * 
 */
...
class SleepMessages implements Runnable {
	...
	public void run() {
		...
		try {
			...
			System.out.println("Thread interrupted: no more messages");
...
public class A2_Thread_Management {
	...
	public static void main(String... args) throws InterruptedException {
		...
		Thread thread = new Thread(new SleepMessages(), "SleepMessages");
		...
		thread.join(5000);// Waits at most 5000 milliseconds for this thread to
							// die.

		if (thread.isAlive()) {

			System.out.println("\nInterrupting the Thread since it has taken more than 5 seconds");
