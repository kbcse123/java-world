...
class SampleProducer implements Runnable {
	...
	public SampleProducer(Container drop) {
...
class SampleConsumer implements Runnable {
	...
	public SampleConsumer(Container drop) {
...
public class Z0_ProducerConsumerVariants {
	...
	private static void usingTraditionalApproach() {
		...
		Thread producerThread = new Thread(new SampleProducer(sharedObject));

		Thread consumerThread = new Thread(new SampleConsumer(sharedObject));
